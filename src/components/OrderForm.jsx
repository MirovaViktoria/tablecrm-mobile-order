...
const OrderForm = () => {
    ...
    const handleSubmit = async (isConducted = false) => {
        if (!client || !selectedWarehouse || !selectedOrganization || !selectedPaybox || !selectedPriceType || cartItems.length === 0) {
            showToast('Пожалуйста, заполните все обязательные поля и добавьте товары', 'warning');
            return;
        }
        ...
        try {
            ...
            showToast(`Заказ ${isConducted ? 'создан и проведен' : 'создан'} успешно!`, 'success');

            setCartItems([]);
            setComment('');
        } catch (error) {
            console.error('Error creating order:', error);
            showToast('Не удалось создать заказ. ' + (error.response?.data?.detail || error.message), 'error');
        } finally {
            setSubmitting(false);
        }
    };
